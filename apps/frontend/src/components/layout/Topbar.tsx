  ...
  return (
    <header className="h-16 border-b border-orbit-border bg-orbit-surface/80 backdrop-blur-xl flex items-center px-6 gap-4 flex-shrink-0 relative z-30">
      ...
      <div className="flex items-center gap-2 text-sm">
        <span className="text-slate-500">Sirekan</span>
        {location.pathname.split('/').filter(x => x).map((path, idx) => (
          <React.Fragment key={path}>
            <span className="text-slate-600">/</span>
            <span className={idx === location.pathname.split('/').filter(x => x).length - 1 ? "text-slate-200 font-medium" : "text-slate-500"}>
              {path.toUpperCase()}
            </span>
          </React.Fragment>
        ))}
