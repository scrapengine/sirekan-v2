...
export const NodeBEditPage: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      const [nodeRes, stoRes] = await Promise.all([client.get(`/api/master-data/nodeb/${id}`), client.get('/api/master-data/sto')]);
      setFormData(nodeRes.data);
      setStoOptions(stoRes.data);
      setLoading(false);
    };
    init();
  }, [id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await client.patch(`/api/master-data/nodeb/${id}`, formData);
    navigate('/wan/nodeb');
  };

  if (loading) return <div className="p-6 bg-orbit-bg h-screen text-slate-400">Loading...</div>;
    ...
    <div className="p-6 h-screen overflow-y-auto bg-orbit-bg text-sm text-slate-100">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <h2 className="font-bold text-base">EDIT DATA NODE-B</h2>
        </div>
        <Button onClick={handleSubmit} className="bg-blue-600 text-white flex items-center gap-2"><Save size={16} /> Save Changes</Button>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Section title="METRO-OLT">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs uppercase text-slate-400">Hostname OLT</label>
              <div className="flex gap-2">
                <input name="hostname_olt" value={formData.hostname_olt || ''} onChange={handleChange} className="flex-1 bg-orbit-surface2 border border-orbit-border p-2 rounded-lg text-sm" />
