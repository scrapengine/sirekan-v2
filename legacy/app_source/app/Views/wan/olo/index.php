            ...
            <div class="card card-primary" id="table-contents">
                <div class=" card-header p-0">
                    <div class="col-10 pr-0">
                        <h4>DATA OLO
                            <?php if (in_groups(['superadmin', 'admin'])) : ?>
                                <a href="<?= base_url('wan/olo/new'); ?> " class="btn btn-sm btn-round btn-primary ml-3" data-toggle="tooltip" data-placement="bottom" title="Add Data"><i class="fas fa-plus"></i></a>
                            <?php endif ?>
                        </h4>
                    </div>
                    <?php if (in_groups(['superadmin', 'admin'])) : ?>
                        <div class="col-2 text-right pl-0">
                            <a href="<?= base_url('wan/olo/trash'); ?> " class="btn btn-sm rounded-pill btn-outline-danger" data-toggle="tooltip" data-placement="bottom" title="Data Trash"><i class="far fa-trash-alt"></i></a>
                        </div>
                    <?php endif ?>
                </div>
                <div class="card-body p-2 mt-3">
                    <div class="dropdown mb-2 text-right">
                        ...
                        <div class="dropdown-menu" role="menu" style="max-height: 250px; overflow-y: scroll;">
                            ...
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "3" : "2" ?>"><span>Layanan</span></a>
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "4" : "3" ?>"><span>Service ID</span></a>
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "5" : "4" ?>"><span>Nama Pelanggan</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "6" : "5" ?>"><span>Alamat</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "7" : "6" ?>"><span>Coordinate</span></a>
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "8" : "7" ?>"><span>Hostname Metro</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "9" : "8" ?>"><span>IP Metro</span></a>
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "10" : "9" ?>"><span>Port Metro</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "11" : "10" ?>"><span>Hostname OLT</span></a>
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "12" : "11" ?>"><span>IP OLT</span></a>
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "13" : "12" ?>"><span>Port Onu</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "14" : "13" ?>"><span>Vlan</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "15" : "14" ?>"><span>Hostname ONT</span></a>
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "16" : "15" ?>"><span>IP ONT</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "17" : "16" ?>"><span>ONT Type</span></a>
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "18" : "17" ?>"><span>Serial Number</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "19" : "18" ?>"><span>ODC</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "20" : "19" ?>"><span>ODP</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "21" : "20" ?>"><span>Description</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "22" : "21" ?>"><span>Bandwidth</span></a>
                        </div>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-striped table-hover table-sm table-bordered" id="oloWan" style="width: 100%">
                            <thead>
                                <tr>
                                    <?php if (in_groups(['superadmin', 'admin'])) : ?>
                                        <th>Action</th>
...
<div class="modal fade" id="table_import">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="/wan/olo/import" method="post" enctype="multipart/form-data">
                <?= csrf_field(); ?>
                <div class="modal-body">
                    <a class="btn btn-sm btn-link float-right text-info" href="<?= base_url('OLO-Format-Import.xlsx'); ?>">
...
<script>
    $(document).ready(function() {
        ...
        table = $('#oloWan').DataTable({
            processing: true,
            serverSide: true,
            searching: true,
            destroy: true,
            ajax: "<?php echo site_url('wan/olo/listdata' . $param) ?>",
            order: [],
            columnDefs: [],
            buttons: [
                'colvis',
        ...
        $("#addColumn").click(function() {
            ...
            $('#penambahanInput').append(
                ...
                '<option value="dataolo.nama">Customer Name</option>' +
                '<option value="dataolo.idsto">STO</option>' +
                '<option value="dataolo.hostname_metro">Hostname Metro</option>' +
                '<option value="dataolo.hostname_olt">Hostname OLT</option>' +
        ...
        $("#exportclose").click(function() {
            ...
            document.getElementById('SearchCloseForm').action = "<?= base_url('wan/olo/export'); ?>";
        ...
        $("#submitSearchClose").click(function() {
            ...
            table = $('#oloWan').DataTable({
                ...
                searching: true,
                destroy: true,
                ajax: "<?php echo site_url('wan/olo/listdata' . $param) ?>" + "?" + $('#SearchCloseForm').serialize(),
                order: [],
                columnDefs: [],
                columns: [
                    <?php if (in_groups(['superadmin', 'admin'])) : ?> {
...
<script>
    ...
    $(document).on('click', '#btnDelete', function(event) {
        ...
        var url = "<?php echo site_url('/wan/olo/') ?>" + idolo

        // console.log(url);
        swal({
                title: 'Delete Data',
                text: 'Apakah anda yakin ingin menghapus ' + service_id + ' - ' + nama + ' ?',
                icon: 'warning',
                buttons: true,
                dangerMode: true,
            })
            .then((isConfirm) => {
                if (isConfirm) {
                    $.ajax({
                        type: "POST",
                        url: "<?php echo site_url('/wan/olo/') ?>" + idolo,
                        data: {
