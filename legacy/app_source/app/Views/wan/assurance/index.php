                ...
                <div class=" card-header p-0">
                    ...
                    <?php if (in_groups(['superadmin', 'admin'])) : ?>
                        <div class="col-2 text-right pl-0">
                            <a href="<?= base_url('wan/assurance/trash'); ?> " class="btn btn-sm rounded-pill btn-outline-danger" data-toggle="tooltip" data-placement="bottom" title="Data Trash"><i class="far fa-trash-alt"></i></a>
                        </div>
                    <?php endif ?>
                </div>
...
<div class="modal fade" id="table_import">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <!-- Modal Header -->
            <form action="/wan/assurance/import" method="post" enctype="multipart/form-data">
...
<script>
    // $(function() {
    //     $('#save_as').on('change', function(e) {
    //         e.stopPropagation();
    //         this.value = this.checked ? "on" : "no";
    //     });
    // })
    $(document).ready(function() {
        ...
        $("#exportclose").click(function() {
            ...
            document.getElementById('SearchCloseForm').action = "<?= base_url('wan/assurance/export'); ?>";
        ...
        $("#submitSearchClose").click(function() {
            ...
            $('#assuranceWan').DataTable({
                ...
                searching: true,
                destroy: true,
                ajax: "<?php echo site_url('wan/assurance/listdata' . $param) ?>" + "?" + $('#SearchCloseForm').serialize(),
                order: [
                    [
                        <?= (in_groups(['superadmin', 'admin'])) ? "6, 'desc'" : "5, 'desc'"; ?>
                    ]
    ...
    function showDetail(idasr) {
        ...
        $.ajax({
            url: "<?= site_url('/wan/assurance/detaildata'); ?>",
            data: "idasr=" + idasr,
            dataType: "html",
...
<script>
    ...
    $(document).on('click', '#btnDelete', function(event) {
        ...
        var url = "<?php echo site_url('/wan/assurance/') ?>" + idasr
        ...
        swal({
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    $.ajax({
                        type: "POST",
                        url: "<?php echo site_url('/wan/assurance/') ?>" + idasr,
                        data: {
