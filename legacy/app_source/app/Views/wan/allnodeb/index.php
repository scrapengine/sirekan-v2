            ...
            <div class="card card-primary" id="table-contents">
                <div class=" card-header p-0">
                    <div class="col-10 pr-0">
                        <h4>DATA NODE-B
                            <?php if (in_groups(['superadmin', 'admin'])) : ?>
                                <a href="<?= base_url('wan/allnodeb/new'); ?> " class="btn btn-sm btn-round btn-primary ml-3" data-toggle="tooltip" data-placement="bottom" title="Add Data"><i class="fas fa-plus"></i></a>
                            <?php endif ?>
                        </h4>
                    </div>
                    <?php if (in_groups(['superadmin', 'admin'])) : ?>
                        <div class="col-2 text-right pl-0">
                            <a href="<?= base_url('wan/allnodeb/trash'); ?> " class="btn btn-sm rounded-pill btn-outline-danger" data-toggle="tooltip" data-placement="bottom" title="Data Trash"><i class="far fa-trash-alt"></i></a>
                        </div>
                    <?php endif ?>
                </div>
                <div class="card-body p-2 mt-3">
                    <div class="dropdown mb-2 text-right">
                        ...
                        <div class="dropdown-menu" role="menu" style="max-height: 250px; overflow-y: scroll;">
                            ...
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "2" : "1" ?>"><span>Site ID</span></a>
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "3" : "2" ?>"><span>Site Name</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "4" : "3" ?>"><span>NSA</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "5" : "4" ?>"><span>Kabupaten</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "6" : "5" ?>"><span>Kecamatan</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "7" : "6" ?>"><span>Kelurahan</span></a>
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "8" : "7" ?>"><span>Coordinate</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "9" : "8" ?>"><span>Tech</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "10" : "9" ?>"><span>BAND_2G</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "11" : "10" ?>"><span>BAND_3G</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "12" : "11" ?>"><span>BAND_4G</span></a>
                            <a type="button" class="dropdown-item toggle-vis " data-column="<?= (in_groups(['superadmin', 'admin'])) ? "13" : "12" ?>"><span>BAND UPDATED</span></a>
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "14" : "13" ?>"><span>TP</span></a>
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "15" : "14" ?>"><span>Source Power</span></a>
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "16" : "15" ?>"><span>Transport</span></a>
                            <a type="button" class="dropdown-item toggle-vis active" data-column="<?= (in_groups(['superadmin', 'admin'])) ? "17" : "16" ?>"><span>STO</span></a>
                        </div>
                    </div>
...
<div class="modal fade" id="table_import">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="/wan/allnodeb/import" method="post" enctype="multipart/form-data">
                <?= csrf_field(); ?>
                <div class="modal-body">
                    <a class="btn btn-sm btn-link float-right text-info" href="<?= base_url('NODEB-ALL-Format-Import.xlsx'); ?>">
...
<script>
    $(document).ready(function() {
        ...
        table = $('#nodebWan').DataTable({
            ...
            searching: true,
            destroy: true,
            ajax: "<?php echo site_url('wan/allnodeb/listdata' . $param) ?>",
            order: [],
            columnDefs: [],
            buttons: [
                'colvis',
        ...
        $("#addColumn").click(function() {
            ...
            $('#penambahanInput').append(
                ...
                '<option value="site_id_all">Site ID</option>' +
                '<option value="site_name_all">Site Name</option>' +
                '<option value="sto">STO</option>' +
                '<option value="tp">TP</option>' +
                '<option value="s_power">Source Power</option>' +
                '</select>' +
                '</div>' +
                '<div class="col-lg-5">' +
        ...
        $("#exportclose").click(function() {
            ...
            document.getElementById('SearchCloseForm').action = "<?= base_url('wan/allnodeb/export'); ?>";
        ...
        $("#submitSearchClose").click(function() {
            ...
            table = $('#nodebWan').DataTable({
                ...
                searching: true,
                destroy: true,
                ajax: "<?php echo site_url('wan/allnodeb/listdata' . $param) ?>" + "?" + $('#SearchCloseForm').serialize(),
                order: [],
                columnDefs: [],
                buttons: [
                    'colvis',
...
<script>
    ...
    $(document).on('click', '#btnDelete', function(event) {
        ...
        var site_id = $(this).data('site_id_all');
        var site_name = $(this).data('site_name_all');
        var url = "<?php echo site_url('/wan/allnodeb/') ?>" + site_id
        ...
        swal({
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    $.ajax({
                        type: "POST",
                        url: "<?php echo site_url('/wan/allnodeb/') ?>" + site_id,
                        data: {
