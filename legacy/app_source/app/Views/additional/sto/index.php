...
<div class="main-content">
    <section class="section">
        <div class="section-header mb-3 px-3">
            <div class="font-weight-bold ml-3" style="font-size: 1rem;">DATA STO</span></div>

            <?php if (in_groups(['superadmin', 'admin'])) : ?>
                <div class="section-header-button ml-2">
                    <a href="<?= base_url('additional/sto/new'); ?> " class="btn btn-sm btn-link" data-toggle="tooltip" data-placement="bottom" title="Add Data"><i class="fas fa-upload"></i></a>
                </div>
                <div class="section-header-breadcrumb col-2 justify-content-end">
                    <div class="btn-group  rounded-pill">
                        ...
                        <a href="<?= base_url('additional/sto/trash'); ?> " class="btn btn-sm btn-link text-danger" data-toggle="tooltip" data-placement="bottom" title="Data Trash"><i class="far fa-trash-alt"></i></a>
                    </div>
                </div>
            <?php endif ?>
        </div>
            ...
            <div class="card card-primary">
                <div class="card-body p-2 mt-3">
                    <div class="text-right mb-3 pr-3">
                        <a href="<?= site_url('additional/sto/export' . $param); ?>" class="btn btn-link animation-on-hover">
                            <i class="fas fa-download" data-toggle="tooltip" data-placement="left" title="Download"></i>
                        </a>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-striped table-hover table-sm" id="datasto" style="width: 100%">
                            <thead>
                                <tr>
                                    <?php if (in_groups(['superadmin', 'admin'])) : ?>
                                        <th>Action</th>
...
<div class="modal fade" id="table_import">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="/additional/sto/import" method="post" enctype="multipart/form-data">
                <?= csrf_field(); ?>
                <div class="modal-body">
                    <a class="btn btn-sm btn-link float-right text-info" href="<?= base_url('STO-Format-Import.xlsx'); ?>">
...
<script>
    $(document).ready(function() {
        table = $('#datasto').DataTable({
            processing: true,
            serverSide: true,
            searching: true,
            ajax: "<?php echo site_url('additional/sto/listdata' . $param) ?>",
            order: [],
            columnDefs: [],
            columns: [
                <?php if (in_groups(['superadmin', 'admin'])) : ?> {
...
<script>
    ...
    $(document).on('click', '#btnDelete', function(event) {
        ...
        var idsto = $(this).data('idsto');
        var nama_sto = $(this).data('nama_sto');
        var url = "<?php echo site_url('/additional/sto/') ?>" + idsto

        // console.log(url);
        swal({
                title: 'Delete Data',
                text: 'Apakah anda yakin ingin menghapus ' + idsto + ' - ' + nama_sto + ' ?',
                icon: 'warning',
                buttons: true,
                dangerMode: true,
            })
            .then((isConfirm) => {
                if (isConfirm) {
                    $.ajax({
                        type: "POST",
                        url: "<?php echo site_url('/additional/sto/') ?>" + idsto,
                        data: {
