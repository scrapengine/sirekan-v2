...
<div class="main-content">
    <section class="section">
        <div class="section-header mb-3 px-3">
            <div class="font-weight-bold ml-3" style="font-size: 1rem;">DATA OLT</span></div>

            <?php if (in_groups(['superadmin', 'admin'])) : ?>
                <div class="section-header-button ml-2">
                    <a href="<?= base_url('additional/olt/new'); ?> " class="btn btn-sm btn-link" data-toggle="tooltip" data-placement="bottom" title="Add Data"><i class="fas fa-upload"></i></a>
                </div>
            <?php endif ?>
            <div class="section-header-breadcrumb col-2 justify-content-end">
                <div class="btn-group  rounded-pill">
                    ...
                    <?php if (in_groups(['superadmin', 'admin'])) : ?>
                        <a href="<?= base_url('additional/olt/trash'); ?> " class="btn btn-sm btn-link text-danger" data-toggle="tooltip" data-placement="bottom" title="Data Trash"><i class="far fa-trash-alt"></i></a>
                    <?php endif ?>
                </div>
            </div>
        </div>
                                ...
                                <div class="row justify-content-center">
                                    <div class="col-lg-3">
                                        <select class="form-control  d-block m-2 text-primary" name="choice[]" id="choice">
                                            ...
                                            <option value="hostname_metro">Hostname_Metro</option>
            ...
            <div class="card card-primary" id="table-contents">
                <div class="card-body p-2 mt-3">
                    <div class="table-responsive">
                        <table class="table table-striped table-hover table-sm" id="dataOlt" style="width: 100%">
                            <thead>
                                <tr>
                                    <?php if (in_groups(['superadmin', 'admin'])) : ?>
                                        <th>Action</th>
...
<div class="modal fade" id="table_import">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="/additional/olt/import" method="post" enctype="multipart/form-data">
                <?= csrf_field(); ?>
                <div class="modal-body">
                    <a class="btn btn-sm btn-link float-right text-info" href="<?= base_url('OLT-Format-Import.xlsx'); ?>">
...
<script>
    $(document).ready(function() {
        table = $('#dataOlt').DataTable({
            processing: true,
            serverSide: true,
            searching: true,
            destroy: true,
            ajax: "<?php echo site_url('additional/olt/listdata') ?>",
            order: [],
            columnDefs: [],
            columns: [
                <?php if (in_groups(['superadmin', 'admin'])) : ?> {
        ...
        $("#exportclose").click(function() {
            document.getElementById('SearchCloseForm').action = "<?= base_url('additional/olt/export'); ?>";
        ...
        $("#submitSearchClose").click(function() {
            ...
            $('#dataOlt').DataTable({
                ...
                searching: true,
                destroy: true,
                ajax: "<?php echo site_url('additional/olt/listdata' . $param) ?>" + "?" + $('#SearchCloseForm').serialize(),
                order: [],
                columnDefs: [],
                columns: [
                    <?php if (in_groups(['superadmin', 'admin'])) : ?> {
...
<script>
    ...
    $(document).on('click', '#btnDelete', function(event) {
        ...
        var url = "<?php echo site_url('/additional/olt/') ?>" + hostname_olt

        // console.log(url);
        swal({
                title: 'Delete Data',
                text: 'Apakah anda yakin ingin menghapus ' + hostname_olt + ' ?',
                icon: 'warning',
                buttons: true,
                dangerMode: true,
            })
