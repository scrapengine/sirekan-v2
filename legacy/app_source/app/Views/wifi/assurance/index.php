            ...
            <div class="card card-primary" id="table-contents">
                <div class=" card-header p-0">
                    <div class="col-10 pr-0">
                        <h4>DATA ASSURANCE
                            <?php if (in_groups(['superadmin', 'admin'])) : ?>
                                <a href="<?= base_url('wifi/assurance/new'); ?> " class="btn btn-sm btn-round btn-primary ml-3" data-toggle="tooltip" data-placement="bottom" title="Add Data"><i class="fas fa-plus"></i></a>
                            <?php endif ?>
                        </h4>
                    </div>
                    <?php if (in_groups(['superadmin', 'admin'])) : ?>
                        <div class="col-2 text-right pl-0">
                            <a href="<?= base_url('wifi/assurance/trash'); ?> " class="btn btn-sm rounded-pill btn-outline-danger" data-toggle="tooltip" data-placement="bottom" title="Data Trash"><i class="far fa-trash-alt"></i></a>
                        </div>
                    <?php endif ?>
                </div>
                <div class="card-body p-2 mt-3">
                    <div class="table-responsive">
                        <table class="table table-striped table-hover table-sm table-bordered" id="assuranceWifi" style="width: 100%">
                            <thead>
                                <tr>
                                    <?php if (in_groups(['superadmin', 'admin'])) : ?>
                                        <th>Action</th>
...
<div class="modal fade" id="table_import">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="/wifi/assurance/import" method="post" enctype="multipart/form-data">
                <?= csrf_field(); ?>
...
<script>
    $(document).ready(function() {
        ...
        table = $('#assuranceWifi').DataTable({
            processing: true,
            serverSide: true,
            searching: true,
            destroy: true,
            ajax: "<?php echo site_url('wifi/assurance/listdata' . $param) ?>",
            order: [
                [
        ...
        $("#exportclose").click(function() {
            ...
            document.getElementById('SearchCloseForm').action = "<?= base_url('wifi/assurance/export'); ?>";
        ...
        $("#submitSearchClose").click(function() {
            ...
            $('#assuranceWifi').DataTable({
                processing: true,
                serverSide: true,
                searching: true,
                destroy: true,
                ajax: "<?php echo site_url('wifi/assurance/listdata' . $param) ?>" + "?" + $('#SearchCloseForm').serialize(),
                order: [
                    [
                        <?= (in_groups(['superadmin', 'admin'])) ? "6, 'desc'" : "5, 'desc'"; ?>
                    ]
    ...
    function showDetail(idasr) {
        ...
        $.ajax({
            url: "<?= site_url('/wifi/assurance/detaildata'); ?>",
            data: "idasr=" + idasr,
            dataType: "html",
...
<script>
    ...
    $(document).on('click', '#btnDelete', function(event) {
        ...
        var url = "<?php echo site_url('/wifi/assurance/') ?>" + idasr
        ...
        swal({
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    $.ajax({
                        type: "POST",
                        url: "<?php echo site_url('/wifi/assurance/') ?>" + idasr,
                        data: {
